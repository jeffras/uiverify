...
namespace VisualUIAVerify.Controls
{
    ...
    public partial class AutomationElementTreeControl : UserControl
    {
        ...
        {
            InitializeComponent();
            generateToolStripMenuItem.Enabled = MainWindow.CodeGeneraters.Count > 0;

            foreach (var codeGenerater in MainWindow.CodeGeneraters)
            {
                var generaterMenu = new ToolStripMenuItem(codeGenerater.Name);
                generaterMenu.Click += (o, e) =>
                    {
                        var form = new GeneratedResultsForm();
                        form.codeTextBox.Text = codeGenerater.Generate(SelectedNode);
                        form.LoadClipboard();
                        form.ShowDialog();
                    };
                generateToolStripMenuItem.DropDownItems.Add(generaterMenu);
            }
        }
